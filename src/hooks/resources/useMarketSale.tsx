...
export function useMarketSale({ pagination, filter }: UseMarketSaleProps): UseMarketSaleResult {
  const queryClient = useQueryClient();

  const { data, isLoading, isIdle } = useQuery(["market-pools", { pagination, filter }], () =>
    MarketSaleApi.getMarketSale({ body: { ...pagination, filters: filter } }),
  );
  ...
  function invalidateDependentQuery() {
    queryClient.invalidateQueries(["market-pools", { pagination, filter }]);
  }
